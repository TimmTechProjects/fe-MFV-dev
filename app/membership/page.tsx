...
import { Check, X, ChevronDown, ChevronUp, Leaf, Shield, Sparkles } from "lucide-react";
...
const comparisonFeatures = [
  { feature: "Plant storage", free: "25 plants", pro: "Unlimited", premium: "Unlimited" },
  { feature: "Public collections", free: true, pro: true, premium: true },
  { feature: "Marketplace access", free: true, pro: true, premium: true },
  { feature: "Community comments", free: true, pro: true, premium: true },
  { feature: "Sell on marketplace", free: false, pro: true, premium: true },
  { feature: "Private collections", free: false, pro: true, premium: true },
  { feature: "Custom profile", free: false, pro: true, premium: true },
  { feature: "Featured listings", free: false, pro: false, premium: true },
  { feature: "Discounted listing fee", free: false, pro: false, premium: true },
  { feature: "Video thumbnails", free: false, pro: false, premium: true },
];
...
const faqs = [
  ...
  },
  {
    ...
    a: "New users get a 14-day free trial of Pro features. No credit card required to start.",
  },
  {
...
const testimonials = [
  {
    ...
    text: "Pro plan transformed how I manage my 200+ plant collection. The private collections feature is a game-changer.",
    plan: "Pro",
  },
  {
    name: "Marcus T.",
    role: "Marketplace Seller",
    text: "Premium featured listings doubled my marketplace sales in the first month. Worth every penny.",
    plan: "Premium",
  },
  {
    name: "Lily R.",
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="relative overflow-hidden py-16 md:py-24">
        <div className="absolute inset-0 bg-gradient-to-br from-green-900/20 via-black to-emerald-900/10" />
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          <span className="inline-block px-4 py-1.5 rounded-full bg-[#81a308]/10 text-[#81a308] text-sm font-medium mb-6 border border-[#81a308]/20">
            Choose Your Plan
          </span>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
            Grow Your Collection,{" "}
            <span className="text-[#81a308]">Your Way</span>
          </h1>
          <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto mb-8">
            From casual hobbyist to professional seller, we have a plan that fits your botanical journey.
          </p>

          <div className="inline-flex items-center gap-3 bg-gray-900 rounded-full p-1.5 border border-gray-800">
            <button
              onClick={() => setIsYearly(false)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${
                !isYearly
                  ? "bg-[#81a308] text-black"
      ...
      <div className="max-w-7xl mx-auto px-6 pb-16">
          ...
          {membershipTiers.map((tier) => (
            <Card
              ...
              className={`relative border bg-[#1f1f1f] flex flex-col justify-between transition-all duration-300 hover:translate-y-[-4px] ${
                tier.popular
                  ? "border-[#81a308] shadow-lg shadow-[#81a308]/10 scale-[1.02] md:scale-105"
            ...
            >
              {tier.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="bg-[#81a308] text-black text-xs font-bold px-4 py-1 rounded-full">
                    Most Popular
                  </span>
                </div>
              )}
              <CardHeader className="text-center pt-8">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[#81a308]/10 text-[#81a308] mx-auto mb-3">
                  {tier.icon}
                </div>
                ...
                {isYearly && tier.price.yearly > 0 && (
                  <p className="text-xs text-[#81a308] mt-1">
              ...
              <CardContent className="flex-1 flex flex-col">
                <ul className="space-y-3 text-sm text-gray-300 flex-1">
                  {tier.benefits.map((benefit, index) => (
                    <li key={index} className="flex items-start gap-2.5">
                      <Check className="w-4 h-4 text-[#81a308] mt-0.5 shrink-0" />
                      <span>{benefit}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-6">
                  {currentPlan === tier.plan ? (
                    <Button
                      disabled
                      className="w-full bg-gray-700 cursor-default rounded-xl py-3"
                    >
                      Current Plan
                    </Button>
                  ) : (
                    <Button
                      className={`w-full rounded-xl py-3 transition-all ${
                        tier.popular
                          ? "bg-[#81a308] hover:bg-[#6ca148] text-black font-semibold"
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="text-center py-4 px-4 text-[#81a308] font-medium">Pro</th>
                  <th className="text-center py-4 px-4 text-gray-400 font-medium">Premium</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    {(["free", "pro", "premium"] as const).map((plan) => {
                      ...
                      return (
                        <td key={plan} className="text-center py-3.5 px-4">
                          {typeof val === "boolean" ? (
                            val ? (
                              <Check className="w-5 h-5 text-[#81a308] mx-auto" />
                            ) : (
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div
                key={i}
                className="bg-[#1f1f1f] border border-gray-800 rounded-xl p-6 hover:border-gray-700 transition-colors"
              >
                <p className="text-gray-300 text-sm leading-relaxed mb-4">
                  &ldquo;{t.text}&rdquo;
                </p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#81a308]/20 flex items-center justify-center text-[#81a308] font-bold text-sm">
                    {t.name[0]}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-white">{t.name}</p>
                    <p className="text-xs text-gray-500">
                      {t.role} &middot; {t.plan} member
                    </p>
                  </div>
                </div>
              </div>
        ...
        {currentPlan !== "free" && (
          <div className="mt-16 text-center border border-gray-800 rounded-2xl p-8 max-w-2xl mx-auto bg-[#1f1f1f]">
            ...
            <Button
              variant="destructive"
              className="rounded-xl px-8 py-2.5 bg-red-600 hover:bg-red-700 text-white"
